...
class InvoiceAdminListTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('invoice_list_admin')
    ...
    def test_invoice_admin_url(self):
        self.assertEqual(self.url, '/invoice/admin')

    def test_get_invoice_admin_view(self):
    ...
    def test_balance_view_displays_all_invoices(self):
        ...
        self.assertTemplateUsed(response, 'invoice_list.html')
        invoices = response.context['invoices']
        self.assertEqual(invoices.all().count(), Invoice.objects.count())